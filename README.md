...
なお、"Slider_and_Motor"についてはパッケージ"python3-tk"（Python3系）もしくは"python-tk"（Python2系）が必要になります。script実行前に、以下のようにコマンドラインからパッケージをインストールしてください。また、"Slider_and_Motor"はDesktop環境のみで実行可能です。Server環境やDocker等のContainer環境を使用されている方はご注意ください。
```bash
